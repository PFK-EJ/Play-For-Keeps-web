...
export default function App() {
    ...
    return (
      <div style={{display:"flex",flexDirection:"column",gap:4,touchAction:"none"}}>
        {fl.map((item, visIdx) => {
                ...
                {isRenaming ? (
                  <>
                    ...
                    <button onPointerDown={e=>e.stopPropagation()} onClick={saveRename}
                      style={{padding:"4px 12px",background:"#DDB34D",border:"none",borderRadius:6,color:"#000",fontWeight:900,cursor:"pointer",fontSize:12}}>Save</button>
          ...
          if (isEd) return (
            <div key={item.id} style={{background:"#1a1200",border:"1px solid #DDB34D",borderRadius:10,padding:"14px 16px"}}>
              ...
              <div style={{display:"flex",gap:8}}>
                <button onClick={savePlayer} style={{padding:"7px 18px",background:"#DDB34D",border:"none",borderRadius:7,color:"#000",fontWeight:900,cursor:"pointer",fontSize:12}}>Save</button>
          ...
          return (
            <div key={item.id} ref={el=>rowRefs.current[item.id]=el}
              ...
              style={{
                ...
                border:`2px solid ${isDragging ? color : isOver ? "#DDB34D77" : "#1e1e1e"}`,
                ...
                boxShadow: isOver?"0 4px 14px #DDB34D22":"none",
              }}>
    ...
    <div style={{display:"flex",gap:6,marginBottom:16,flexWrap:"wrap",alignItems:"center"}}>
      ...
      {["ALL","WR","RB","TE","QB"].map(p=>(
        <button key={p} onClick={()=>setPosFilter(p)} style={{
          ...
          border:posFilter===p?`2px solid ${POS_COLORS[p]||"#DDB34D"}`:"2px solid #2a2a2a",
          background:posFilter===p?(POS_COLORS[p]||"#DDB34D")+"22":"transparent",
          color:posFilter===p?(POS_COLORS[p]||"#DDB34D"):"#777"
        }}>{p}</button>
      ))}
    </div>
  );
  ...
  const grade=(()=>{let s=0;s+=Math.min(eliteCount*15,45);s+=Math.min(teamRoster.filter(r=>Number(r.age||99)<=24).length*5,20);s+=Math.min(picks.length*5,20);s+=Number(avgAge)<=24?15:Number(avgAge)<=26?8:0;if(s>=75)return{g:"A",l:"Championship Window",c:"#DDB34D"};if(s>=60)return{g:"B+",l:"Contender",c:"#FFC107"};if(s>=45)return{g:"B",l:"Rising",c:"#e0a800"};if(s>=30)return{g:"C+",l:"Transitioning",c:"#b8860b"};return{g:"C",l:"Rebuild",c:"#777"};})();

  return (
    <div style={{background:"#080808",minHeight:"100vh",color:"#f0f0f0",fontFamily:"'Inter','Segoe UI',sans-serif"}}>
      <div style={{background:"linear-gradient(135deg,#0a0a0a,#1a1200,#0a0a0a)",borderBottom:"2px solid #DDB34D",padding:"12px 20px"}}>
        <div style={{maxWidth:1140,margin:"0 auto",display:"flex",alignItems:"center",gap:14,flexWrap:"wrap"}}>
          ...
          <div>
            <div style={{fontSize:21,fontWeight:900,color:"#DDB34D",letterSpacing:3,textShadow:"0 0 20px #DDB34D55"}}>PLAY FOR KEEPS</div>
          ...
          <div style={{marginLeft:"auto",display:"flex",gap:6,flexWrap:"wrap"}}>
            {[["pfk","👑 PFK Ranks"],["custom","✏️ My Rankings"],["team","📋 My Team"],["outlook","🔮 Outlook"]].map(([t,l])=>(
              <button key={t} onClick={()=>setTab(t)} style={{padding:"8px 14px",borderRadius:8,border:tab===t?"2px solid #DDB34D":"2px solid #2a2a2a",background:tab===t?"#DDB34D":"transparent",color:tab===t?"#000":"#999",fontWeight:700,fontSize:12,cursor:"pointer",textTransform:"uppercase",letterSpacing:1}}>{l}</button>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{maxWidth:1140,margin:"0 auto",padding:"20px 14px"}}>
        ...
        {tab==="pfk" && (
          <div>
            <div style={{background:"linear-gradient(90deg,#1a1200,#0a0a0a)",border:"1px solid #DDB34D33",borderRadius:12,padding:"14px 20px",marginBottom:18,display:"flex",alignItems:"center",gap:12}}>
              <span style={{fontSize:18}}>👑</span>
              <div>
                <div style={{fontSize:14,fontWeight:900,color:"#DDB34D",letterSpacing:1}}>PLAY FOR KEEPS OFFICIAL RANKINGS</div>
        ...
        {tab==="custom" && (
          <div>
            <div style={{background:"#111",border:"1px solid #DDB34D55",borderRadius:12,padding:"12px 18px",marginBottom:14,display:"flex",alignItems:"center",gap:10,flexWrap:"wrap"}}>
              <span style={{fontSize:13,fontWeight:800,color:"#DDB34D"}}>✏️ My Rankings</span>
              <span style={{fontSize:11,color:"#666",flex:1}}>Drag ⠿ to move players or tier banners freely · tiers auto-assign</span>
              <button onClick={()=>setShowAddTier(v=>!v)} style={{padding:"6px 12px",background:"#1a1200",border:"1px solid #DDB34D",borderRadius:7,color:"#DDB34D",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Tier</button>
              ...
              <button onClick={undo} disabled={!history.length} style={{padding:"6px 12px",background:"transparent",border:`1px solid ${history.length?"#DDB34D":"#333"}`,borderRadius:7,color:history.length?"#DDB34D":"#444",fontWeight:700,cursor:history.length?"pointer":"default",fontSize:12}}>↩ Undo</button>
            ...
            {showAddTier && (
              <div style={{background:"#111",border:"1px solid #DDB34D",borderRadius:10,padding:14,marginBottom:12,display:"flex",gap:8,alignItems:"center"}}>
                ...
                <button onClick={addTier} style={{padding:"7px 16px",background:"#DDB34D",border:"none",borderRadius:7,color:"#000",fontWeight:900,cursor:"pointer",fontSize:12}}>Add Tier</button>
            ...
            {showAdd && (
              <div style={{background:"#111",border:"1px solid #DDB34D",borderRadius:10,padding:14,marginBottom:12,display:"flex",gap:8,flexWrap:"wrap",alignItems:"flex-end"}}>
                ...
                <button onClick={addNewPlayer} style={{padding:"7px 16px",background:"#DDB34D",border:"none",borderRadius:7,color:"#000",fontWeight:900,cursor:"pointer",fontSize:12}}>Add</button>
        ...
        {tab==="team" && (
          <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))",gap:20}}>
            <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:12,padding:20}}>
              <div style={{fontSize:13,fontWeight:900,color:"#DDB34D",marginBottom:14,textTransform:"uppercase",letterSpacing:1}}>📋 Roster</div>
              ...
              <div style={{display:"flex",gap:6,flexWrap:"wrap"}}>
                ...
                <button onClick={()=>{if(!newTeamPlayer.name)return;setTeamRoster([...teamRoster,{...newTeamPlayer}]);setNewTeamPlayer({name:"",pos:"WR",age:"",tier:""});}}
                  style={{padding:"7px 13px",background:"#DDB34D",border:"none",borderRadius:7,color:"#000",fontWeight:900,cursor:"pointer",fontSize:12}}>+ Add</button>
              </div>
            </div>
            <div style={{display:"flex",flexDirection:"column",gap:16}}>
              <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:12,padding:20}}>
                <div style={{fontSize:13,fontWeight:900,color:"#DDB34D",marginBottom:14,textTransform:"uppercase",letterSpacing:1}}>🎯 Draft Capital</div>
                ...
                <div style={{display:"flex",flexDirection:"column",gap:6,marginBottom:12}}>
                  {picks.map((p,i)=>(
                    <div key={i} style={{display:"flex",alignItems:"center",gap:10,padding:"7px 12px",background:"#080808",borderRadius:7,border:"1px solid #181818"}}>
                      <span style={{fontSize:13,fontWeight:700,color:"#DDB34D"}}>{p.year}</span>
                ...
                <button onClick={()=>setPicks([...picks,{year:2027,round:"1st",est:"Mid"}])}
                  style={{padding:"7px 13px",background:"transparent",border:"1px solid #DDB34D",borderRadius:7,color:"#DDB34D",fontWeight:700,cursor:"pointer",fontSize:12}}>+ Add Pick</button>
              </div>
              <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:12,padding:20}}>
                <div style={{fontSize:13,fontWeight:900,color:"#DDB34D",marginBottom:14,textTransform:"uppercase",letterSpacing:1}}>📊 Team Grades</div>
                <div style={{display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10}}>
                  {[{l:"Grade",v:grade.g,c:grade.c,s:grade.l},{l:"Avg Age",v:avgAge,c:Number(avgAge)<=24?"#10b981":Number(avgAge)<=26?"#DDB34D":"#ef4444",s:"of roster"},{l:"Elite Assets",v:eliteCount,c:"#DDB34D",s:"Top tiers"},{l:"Draft Picks",v:picks.length,c:"#3b82f6",s:"banked"}].map(({l,v,c,s})=>(
        ...
        {tab==="outlook" && (
          <div style={{display:"flex",flexDirection:"column",gap:18}}>
            ...
            <div style={{background:"#0f0f0f",border:"1px solid #1e1e1e",borderRadius:12,padding:22}}>
              <div style={{fontSize:12,fontWeight:900,color:"#DDB34D",marginBottom:12,textTransform:"uppercase",letterSpacing:1}}>📈 Position Breakdown</div>
              {["WR","RB","TE","QB"].map(pos=>{
                const ps=teamRoster.filter(p=>p.pos===pos);
                const v=ps.length===0?{t:"⚠️ No players",c:"#ef4444"}:ps.length>=3?{t:"✅ Good depth",c:"#10b981"}:ps.length>=1?{t:"🟡 Thin — need depth",c:"#DDB34D"}:{t:"🔴 Missing",c:"#ef4444"};
                return(
